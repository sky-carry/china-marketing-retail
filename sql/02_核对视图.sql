...
-- 应 ≤ 该公司【每一个】网点的平台库存；未上架视为不足。京东、美团网点合并核对。
-- 关联：feishu_store_mapping.customer_name = 网点表.dealer（精确匹配，
...
SELECT '京东'::text AS platform,
       o.customer_name, o.product_code, o.product_name,
...
SELECT '美团'::text,
       ...
       mo.store_name,
       msi.stock_qty,
       COALESCE(msi.stock_qty, 0) - o.offline_qty,
       COALESCE(msi.stock_qty >= o.offline_qty, false)
FROM offline o
JOIN feishu_meituan_outlet mo
  ON mo.dealer = o.customer_name AND mo.business_status = '营业中'
LEFT JOIN meituan_store_inventory msi
  ON msi.store_id = mo.store_id
 AND msi.internal_sku_code = o.product_code;

COMMENT ON VIEW v_outlet_guard IS '网点库存保障明细：公司×货号×网点（京东启用+美团营业中），网点平台库存应≥公司线下伯俊总和（A口径），outlet_qty NULL=未上架';
...
SELECT customer_name, product_code,
       ...
       count(*) FILTER (WHERE platform = '京东') AS jd_cnt,
       count(*) FILTER (WHERE platform = '京东' AND is_ok) AS jd_ok,
       count(*) FILTER (WHERE platform = '美团') AS mt_cnt,
       count(*) FILTER (WHERE platform = '美团' AND is_ok) AS mt_ok,
       min(COALESCE(outlet_qty, 0)) AS min_outlet_qty,
       min(gap)           AS worst_gap
FROM v_outlet_guard
GROUP BY 1, 2;

COMMENT ON VIEW v_outlet_guard_summary IS '网点库存保障汇总：公司×货号，总/分平台达标网点数、最小网点库存、最大缺口';
